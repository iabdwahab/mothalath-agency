  ...
  return (
    <>
      <li key={navLink.id} className="max-md:w-full">
        <a href={navLink.link} className={`hover:text-supporter duration-medium font-bold block max-md:py-5 max-md:w-full max-md:text-center ${index !== 0 ? 'max-md:border-t' : ''}`}>
          {navLink.text}
        </a>
      </li>
    </>
